...
if __name__ == '__main__':
    ...
    if mode == 'train':
        ...
        torch.save(model.state_dict(), 'VH.pt')

        #open('more.txt', 'w').write(decode(m.generate(context, max_new_tokens=10000)[0].tolist()))
        sequence_length = 12
        # Create a dummy input tensor
        dummy_input = torch.randint(vocab_size, (batch_size, sequence_length), dtype=torch.int32).to(device)
        #dummy_input = torch.randint(vocab_size, (batch_size, block_size), dtype=torch.int32).to(device)
        # Export the model to ONNX format
        torch.onnx.export(m, dummy_input, "gpt_language_model.onnx", export_params=True, input_names=['inputs'], output_names=['outputs'], opset_version=10)
